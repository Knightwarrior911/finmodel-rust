...
- **Authenticated, owned OMP services.** Finmodel now accepts only the supported OMP 17.x gateway with bearer authentication enabled, authenticates every catalog/chat request with OMP's private token, rejects unexpected loopback services with the real validation error instead of reopening login, and stops only the broker/gateway processes it started when the app exits.
- **Provider readiness now means a live provider.** Stored Cursor/OpenCode credentials are diagnostic metadata, not proof of readiness: finmodel projects only presence/expiry metadata from OMP's database, accepts refreshable Cursor OAuth, sends expired non-refreshable logins back through Connect, preserves a saved provider's identity while its gateway is stopped, keeps every catalog scoped, and invalidates stale capability data whenever the provider, model, or key changes.
- **No more successful `Done` turns.** The streaming seam preserves reasoning across tool continuations, preserves normalized SSE error routing, recognizes punctuation- or emoji-decorated status replies, and applies one explicit answer retry to empty or status-only output after primary, provider-recovery, synthesis-finisher, and terminal wrap-up requests. The retry instruction is request-local and never enters durable conversation history.
