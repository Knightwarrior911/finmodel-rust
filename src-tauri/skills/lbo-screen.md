...
---
1. Pull the target's financials with `get_financials` (revenue, operating income → proxy EBITDA if D&A unavailable, net income) and market pricing with `get_quote`. Use `read_filing` (MD&A / risk factors) to check cash-flow stability and existing debt commentary.
