...
use crate::model::{Cell, DATA0, LABEL, Sheet, TAN, Value, cell_ref};
...
pub fn build_ev_bridge_sheet(inp: &EvBridgeInput, generated: &str) -> Sheet {
    ...
    let name = if inp.company.is_empty() {
        "Company"
    } else {
        &inp.company
    };
    ...
    let curr = if inp.currency.is_empty() {
        "USD"
    } else {
        &inp.currency
    };
    ...
    input(
        &mut s,
        row,
        "Shares Outstanding (wtd avg basic)",
        inp.shares_outstanding.unwrap_or(0.0),
        NF_SHARES,
    ...
    formula(
        ...
        &format!("={}*{}/1000000", d(r_price), d(r_shares)),
        NF_DOLLAR,
    );
    row += 1;
    row += 1; // spacer
    ...
    let add_map: [(Option<f64>, &str, &str); 6] = [
        ...
        (
            ...
            "Operating Lease Liabilities (R-016)",
            "ASC 842 / IFRS 16 lease footnote (R-016)",
        ),
        (
            inp.underfunded_pension,
            "Underfunded Pension (R-015)",
            "Pension footnote ONLY — NOT balance sheet (R-015)",
        ),
        (
            inp.minority_interest,
    ...
    let sub_map: [(Option<f64>, &str, &str); 7] = [
        ...
        (
            ...
            "Equity Method Investments (R-014)",
            "Balance Sheet — non-operating (R-014)",
        ),
        (
...
pub fn build_ifrs_bridge_sheet(inp: &IfrsBridgeInput, generated: &str) -> Sheet {
    ...
    s.merge(
        ...
        LABEL,
        Cell {
            value: Some(Value::Text(format!(
                "{} {} — {dir_label}",
                inp.company, inp.period
            ))),
            fill: Some(ADHOC_TITLE.to_string()),
            ..Default::default()
        },
    ...
    section(
        &mut s,
        row,
        "EBITDA Derivation — Hierarchy: Adjusted > Reported > Computed",
    );
    row += 1;
    let r_ebit = row;
    input(
        &mut s,
        row,
        "Reported EBIT (from income statement)",
        inp.reported_ebit,
        NF_DOLLAR,
        "",
    );
    ...
    let (r_start, start_label) = if has_adjusted {
        ...
        input(
            &mut s,
            row,
            "Adjusted EBITDA (company-reported, one-offs removed)",
            inp.reported_ebitda,
            NF_DOLLAR,
            "",
        );
        ...
        formula(
            &mut s,
            row,
            "  Difference (one-off items)",
            &format!("={}-{}", d(r_adj), d(r_ebitda_computed)),
            NF_PLAIN,
        );
        row += 1;
        line(
            &mut s,
            row,
            "  (Adjusted EBITDA NOT related to IFRS 16 — one-off items only)",
        );
        row += 1;
        row += 1; // divider
        ...
        line(
            &mut s,
            row,
            "  (EBITDA not separately reported; using computed EBIT + D&A)",
        );
        row += 1;
    ...
    input(
        &mut s,
        row,
        &format!("  {arrow} ROU Depreciation"),
        inp.rou_depreciation,
        NF_DOLLAR,
        "",
    );
    row += 1;
    ...
    input(
        &mut s,
        row,
        &format!("  {arrow} Interest on Lease Liabilities"),
        inp.lease_interest,
        NF_DOLLAR,
        "",
    );
    ...
    formula(
        ...
        row,
        "Pre-IFRS EBITDA",
        &format!("={}{op}{}{op}{}", d(r_start), d(r_rou), d(r_int)),
        NF_DOLLAR,
    );
    s.stamp_bold_row(row);
    row += 1;
    ...
    if inp.revenue > 0.0 {
        ...
        formula(
            ...
            &format!("={}/{}", d(r_start), d(r_rev.unwrap())),
            NF_PCT,
        );
        row += 1;
        formula(
            &mut s,
            row,
            "  Pre-IFRS EBITDA Margin",
            &format!("={}/{}", d(r_adj_ebitda), d(r_rev.unwrap())),
            NF_PCT,
        );
        row += 1;
    }
    ...
    formula(
        ...
        row,
        "Adjusted EBIT",
        &format!("={}{op}{}", d(r_ebit2), d(r_ebit_int)),
        NF_DOLLAR,
    );
    s.stamp_bold_row(row);
    row += 1;
    ...
    if inp.reported_ebita != 0.0 && inp.reported_ebita != inp.reported_ebit {
        ...
        input(
            &mut s,
            row,
            "Reported EBITA (EBIT + amortisation of intangibles)",
            inp.reported_ebita,
            NF_DOLLAR,
            "",
        );
        ...
        formula(
            ...
            row,
            "Adjusted EBITA",
            &format!("={}{op}{}", d(r_ebita), d(r_ebita_int)),
            NF_DOLLAR,
        );
        s.stamp_bold_row(row);
        row += 1;
        if let Some(rr) = r_rev {
            formula(
                &mut s,
                row,
                "  EBITA Margin (Post-IFRS)",
                &format!("={}/{}", d(r_ebita), d(rr)),
                NF_PCT,
            );
            row += 1;
    ...
    line(
        ...
        &format!(
            "  ROU Depreciation: {}",
            fmt_thousands(inp.rou_depreciation)
        ),
    );
    row += 1;
    line(
        &mut s,
        row,
        &format!("  Lease Interest: {}", fmt_thousands(inp.lease_interest)),
    );
    row += 1;
    if inp.short_term_rent > 0.0 {
        line(
            &mut s,
            row,
            &format!("  Short-term Rent: {}", fmt_thousands(inp.short_term_rent)),
        );
        row += 1;
    }
