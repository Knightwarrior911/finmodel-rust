...
pub fn build(input: &WorkbookInput) -> Sheet {
    ...
    for j in 1..n_h {
        h_wc_ar[j] = round2(
            -(g(bs, "accounts_receivable", j).unwrap_or(0.0)
                - g(bs, "accounts_receivable", j - 1).unwrap_or(0.0)),
        );
        h_wc_inv[j] = round2(
            -(g(bs, "inventory", j).unwrap_or(0.0) - g(bs, "inventory", j - 1).unwrap_or(0.0)),
        );
        h_wc_ap[j] = round2(
            g(bs, "accounts_payable", j).unwrap_or(0.0)
                - g(bs, "accounts_payable", j - 1).unwrap_or(0.0),
        );
        h_wc_defrev[j] = round2(
            g(bs, "deferred_revenue_current", j).unwrap_or(0.0)
    ...
    let h_fx: Vec<f64> = (0..n_h)
        ...
        .collect();
    let h_beg0 = g(bs, "cash", 0).map(|c| round2(c - g(cf, "net_change_cash", 0).unwrap_or(0.0)));
    ...
    for j in n_h..n {
        ...
        s.formula(
            13,
            c,
            format!("=-(BS!{}-BS!{})", cell_ref(12, c), cell_ref(12, p)),
        );
        s.formula(
            14,
            c,
            format!("=-(BS!{}-BS!{})", cell_ref(13, c), cell_ref(13, p)),
        );
        s.formula(
            15,
            c,
            format!("=BS!{}-BS!{}", cell_ref(21, c), cell_ref(21, p)),
        );
        s.formula(
            16,
            c,
            format!("=BS!{}-BS!{}", cell_ref(23, c), cell_ref(23, p)),
        );
        s.number(17, c, 0.0);
    }
    ...
    for j in n_h..n {
        ...
        s.formula(
            22,
            c,
            format!("=-IS!{}*{}", cell_ref(is_rev, c), cell_ref(23, c)),
        );
    }
    ...
    for j in 0..n_p {
        s.formula(
            23,
            col(n_h + j),
            format!("=Assumptions!{}", cell_ref(19, DATA0 + j as u32)),
        );
    }
    ...
    for j in 0..n_h {
        ...
        s.formula(
            ...
            c,
            format!(
                "=IF(IS!{sh}<>0,ABS({div})/IS!{sh},0)",
                sh = cell_ref(is_shares, c),
                div = cell_ref(29, c)
            ),
        );
    }
    for j in 0..n_p {
        ...
        s.formula(
            30,
            c,
            format!("=Assumptions!{}", cell_ref(25, DATA0 + j as u32)),
        );
        s.formula(
            29,
            c,
            format!("=IS!{}*{}", cell_ref(is_shares, c), cell_ref(30, c)),
        );
    }
    ...
    for j in 0..n {
        ...
        s.formula(
            38,
            c,
            format!("={}-ABS({})", cell_ref(19, c), cell_ref(22, c)),
        );
    }
    ...
    for j in 0..n {
        ...
        s.formula(
            40,
            c,
            format!("=ROUND({}-IS!{},2)", cell_ref(11, c), cell_ref(is_ni, c)),
        );
    }
    ...
    for j in 0..n {
        ...
        s.formula(
            41,
            c,
            format!("=ROUND({}-BS!{},2)", cell_ref(37, c), cell_ref(11, c)),
        );
    }
    ...
    for &(row, key) in &[
        ...
        let vals = cf.get(key).or_else(|| is.get(key)).or_else(|| bs.get(key));
