...
pub fn builtin_workflows() -> Vec<WorkflowSpec> {
    vec![
        ...
        WorkflowSpec {
            ...
            allowed_tools: &["benchmark_peers", "get_quote", "list_filings", "read_filing", "build_model", "draft_memo"],
            default_confidentiality: Confidentiality::Confidential,
            ...
            needs_verification: true,
            approval_policy: ApprovalPolicy::NewVersionAuto,
            plan_template: "Assemble peer set → per-peer metrics → normalized comps table (as-of dated) → draft the comps note.",
