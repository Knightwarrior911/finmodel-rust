...
fn slide0_elem(deck: &str, name: &str) -> Value {
    ...
    els.into_iter().find(|e| e["name"] == name).unwrap_or(Value::Null)
}
...
fn move_shape_updates_position() {
    ...
    fm_pptx::edit::move_shape(&deck, 0, Some(2), None, Some(2.5), Some(3.0), None, None, None).unwrap();
    let el = slide0_elem(&deck, "Title");
    assert!((el["pos"]["left"].as_f64().unwrap() - 2.5).abs() < 1e-3, "left {}", el["pos"]["left"]);
    assert!((el["pos"]["top"].as_f64().unwrap() - 3.0).abs() < 1e-3, "top {}", el["pos"]["top"]);
    cleanup(&deck);
}
...
fn delete_shape_removes_it() {
    ...
    let before = fm_pptx::inspect::inspect_pptx(&deck).unwrap()["slides"][0]["elementCount"].as_u64().unwrap();
    fm_pptx::edit::delete_shape(&deck, 0, Some(2), None, None).unwrap();
    let after = fm_pptx::inspect::inspect_pptx(&deck).unwrap()["slides"][0]["elementCount"].as_u64().unwrap();
...
fn add_textbox_appends_shape() {
    ...
    fm_pptx::edit::add_textbox(&deck, 0, 1.0, 5.0, 3.0, 0.5, "Injected note", Some("Note"), None).unwrap();
...
fn table_deck(tag: &str) -> String {
    let pkg = fm_pptx::writer::pkgbuild::build_package(13.333, 7.5, &[vec![TABLE_SHAPE.to_string()]]);
